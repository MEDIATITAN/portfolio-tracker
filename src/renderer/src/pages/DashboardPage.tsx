...
import { assetClassBreakdown, positionBreakdown, regionBreakdown, sectorBreakdown, type RegionGrouping } from '../lib/selectors'
...
import { useFxRates, useHistoricalData, usePositions, usePriceCache, useRefreshPrices, useSnapshots } from '../lib/queries'
...
export function DashboardPage({ classFilter, onNavigateToPositions }: DashboardPageProps): React.JSX.Element {
  ...
  const filteredValues = classFilter === 'ALL' ? values : values.filter((v) => v.position.assetClass === classFilter)
  ...
  function toggle(key: ChartKey): () => void {
    return () => setExpandedChart((prev) => (prev === key ? null : key))
  }

  function toggleHero(key: HeroKey): () => void {
    return () => setExpandedHero((prev) => (prev === key ? null : key))
  }
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 p-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row">
        {classFilter === 'ALL' && (
          <div style={flexStyle('assetClass', expandedChart)} className="min-w-0 transition-all duration-300">
            <BreakdownDonut
              title="Anlageklassen"
        ...
        <div style={flexStyle('sector', expandedChart)} className="min-w-0 transition-all duration-300">
          <BreakdownDonut
            title="Sektoren (Aktien & ETFs)"
            data={sectorBreakdown(filteredValues)}
            expanded={expandedChart === 'sector'}
            onToggleExpand={toggle('sector')}
          />
        </div>
        <div style={flexStyle('region', expandedChart)} className="min-w-0 transition-all duration-300">
          <BreakdownDonut
            title="Regionen (Aktien & ETFs)"
            data={regionBreakdown(filteredValues, regionGrouping)}
            expanded={expandedChart === 'region'}
            onToggleExpand={toggle('region')}
            headerExtra={
              <button
                type="button"
                onClick={() => setRegionGrouping((g) => (g === 'country' ? 'continent' : 'country'))}
